...
import { AI_CHUNK_PLAN_VERSION, AI_MAX_DOCUMENT_ROWS, AI_MAX_DOCUMENT_SOURCE_BYTES, AI_MAX_REQUEST_BYTES, AI_MAX_SOURCE_ITEM_BYTES, AI_MAX_STEERING_BYTES, AI_MAX_TRANSLATED_ITEM_BYTES, type ChunkPlan, type DerivedLayer, type EnumeratedLine, type ModelLimits, type PlannedChunk, type SoundOrthography } from "./types.ts";
...
export const AI_SOUND_SYSTEM_PROMPT = "Return JSON only with shape {\"items\":[{\"id\":string,\"t\":string}]}. Return every requested id exactly once. Write how each lyric is pronounced in the requested target orthography; preserve words already readable in that orthography and do not translate meaning. Preserve the exact ' / ' delimiter count and ordered segment count. Do not add ids, omit ids, merge rows, split rows, number output, or use Markdown.";
...
function createChunk(id: string, target: string, items: ReadonlyArray<{ id: string; c: "ordinary" | "adlib"; s: string }>, model: ModelLimits, instructions?: string, layer: DerivedLayer = "meaning"): PlannedChunk {
...
export function planChunks(rows: ReadonlyArray<EnumeratedLine>, target: string, model: ModelLimits, instructions?: string, layer: DerivedLayer = "meaning"): ChunkPlan {
  ...
  if (sent.length <= 128 && sentBytes <= 16 * 1024 && Math.ceil(sentBytes / 2) <= 6_144) {
    try { return { version: AI_CHUNK_PLAN_VERSION, chunks: [createChunk("C0", target, sent, model, instructions, layer)], enumerableRows: rows.length, canonicalSourceUtf8Bytes }; } catch (error) { if (!(error instanceof RangeError)) throw error; }
  }
  const chunks: PlannedChunk[] = [];
  let current: typeof sent = [];
  let currentBytes = 0;
  const close = () => { if (!current.length) return; chunks.push(createChunk(`C${chunks.length}`, target, current, model, instructions, layer)); current = []; currentBytes = 0; };
  for (const item of sent) {
    const itemBytes = utf8Bytes(item.s);
    const candidate = [...current, item];
    let fits = candidate.length <= 64 && currentBytes + itemBytes <= 8 * 1024;
    if (fits) { try { createChunk("probe", target, candidate, model, instructions, layer); } catch { fits = false; } }
    if (!fits) close();
    current.push(item); currentBytes += itemBytes;
    try { createChunk("probe", target, current, model, instructions, layer); } catch { throw new RangeError("oversized"); }
  }
  close();
...
export function validateProviderItems(items: unknown, requested: ReadonlyArray<{ id: string; c: "ordinary" | "adlib"; s: string }>, layer: DerivedLayer = "meaning", target = "en"): Array<{ id: string; t: string }> {
  ...
  for (const item of items) {
    ...
    if (layer === "meaning" && source.c === "ordinary" && !isMeaningfullyDifferent(item.t, source.s)) throw new TypeError("unchanged_ordinary");
