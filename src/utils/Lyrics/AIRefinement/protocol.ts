...
export const AI_ITERATION_SYSTEM_PROMPT = "Use the additional instructions as the quality target and re-evaluate the complete accepted document in p against the canonical source s. Improve any wording that is materially inaccurate, awkward, inconsistent, or off-target, even when the instructions do not name that exact line. Retain wording that already meets the target when an alternative would not be a real improvement. Do not make text more literal, formal, mechanical, verbose, or merely different for the sake of rewriting it. Return a complete replacement document, not a patch, critique, explanation, or continuation. Do not ask questions.";
...
export function validateProviderItems(items: unknown, requested: ReadonlyArray<ProviderRequestItem>, layer: DerivedLayer = "meaning", target = "en", allowUnchangedIds: ReadonlySet<string> = new Set()): Array<{ id: string; t: string }> {
  ...
  for (const item of items) {
    ...
    if (layer === "meaning" && source.c === "ordinary" && !allowUnchangedIds.has(item.id) && !isMeaningfullyDifferent(item.t, source.s)) throw new TypeError("unchanged_ordinary");
