...
export async function buildDocumentDigest(rows: ReadonlyArray<EnumeratedLine>, context: Partial<LyricContext> | null = null, includeBaseline = false): Promise<string> {
  return sha256Hex({ context: normalizeLyricContext(context), rows: rows.map(({ id, class: lineClass, sendDisposition, sourceText, voice, allowUnchanged, baselineTranslatedText, baselineProvenance }) => ({ id, class: lineClass, sendDisposition, sourceText, voice, allowUnchanged, ...(includeBaseline ? { baselineTranslatedText: baselineTranslatedText ?? null, baselineProvenance: baselineProvenance ?? null } : {}) })) });
}
export async function buildConfigId(value: { layer?: DerivedLayer; provider: string; providerVersion: string; endpoint?: string | null; modelName: string; targetLang: string; sourceLanguage?: string | null; pronunciationSystem?: string | null; soundMode?: "whole_line_v1" | null; soundBaselineMode?: "existing_output_v1" | "raw_source_v1" | null; instructions?: string; promptVersion: number; iterationPromptVersion?: number | null; parentRecordKey?: string | null; parentOutputDigest?: string | null; revisionInstructions?: string | null; temperature: 0; contextMode: "document_or_v1_chunks" }): Promise<string> { return sha256Hex({ ...value, layer: value.layer ?? "meaning", endpoint: value.endpoint ?? null, sourceLanguage: value.sourceLanguage ?? null, pronunciationSystem: value.pronunciationSystem ?? null, soundMode: value.soundMode ?? null, soundBaselineMode: value.soundBaselineMode ?? null, instructions: normalizeSteeringInstructions(value.instructions), iterationPromptVersion: value.iterationPromptVersion ?? null, parentRecordKey: value.parentRecordKey ?? null, parentOutputDigest: value.parentOutputDigest ?? null, revisionInstructions: value.revisionInstructions ? normalizeSteeringInstructions(value.revisionInstructions) : null }); }
...
export function planChunks(rows: ReadonlyArray<EnumeratedLine>, target: string, model: ModelLimits, instructions?: string, layer: DerivedLayer = "meaning", rawContext: Partial<LyricContext> | null = null, previousById: Readonly<Record<string, string>> | null = null, useSoundBaseline = true): ChunkPlan {
  ...
  const sent = rows.filter((line) => line.sendDisposition === "sent").map((line) => {
    ...
    const previous = previousById?.[line.id] ?? (layer === "sound" && useSoundBaseline ? line.baselineTranslatedText : undefined);
